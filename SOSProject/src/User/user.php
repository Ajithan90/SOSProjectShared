...
$status="";
		...
		if(isset($_GET['ppid'])){
			...
			while($rowLoader = $resLoader->fetch(PDO::FETCH_ASSOC)){
			    $id=$rowLoader['uid'];
			    $fname=$rowLoader['First Name'];
			    $Lname=$rowLoader['Last Name'];	
			    $email=$rowLoader['Email_ID'];
			    $user=$rowLoader['username'];
...
<a href="addnewuser.php" id="myButton" class="fancybox fancybox.ajax">Add</a>
<br/><br/>
...
<?php //Search Start
					...
					$str.="<thead><tr><th>First Name</th><th>Last Name</th><th>Email</th><th>User Name</th><th>Status</th><th>Action</th></tr></thead><tbody>";
						
						
						while($row = $res->fetch(PDO::FETCH_ASSOC)){
						    $str.="<td align='center'>".$row['First_Name']."</td>";
						    $str.="<td align='center'>".$row['Last_Name']."</td>";
							$str.="<td align='center'>".$row['Email_ID']."</td>";
							$str.="<td align='center'>".$row['username']."</td>";
							if($row['Status']==1){
							    $statusdesc='<img   alt="village" src="../../images/Active.png"/>';
							}
							else {
							    $statusdesc='<img   alt="village" src="../../images/deactive.png"/>';
							}
							$str.="<td align='center'>".$statusdesc."</td>";
							...
							href='updateuser.php?ppid=".$row['uid']."' onclick='return update()'>
							<img src = '../../images/update.png' alt = 'edit' title = 'edit'/></a>
							


                            <a href='deleteuser.php?pid=".$row['uid']."' onclick='return bura()' >
